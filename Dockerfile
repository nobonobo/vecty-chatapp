...
RUN go get github.com/nobonobo/vecty-sample
RUN cd $(go env GOPATH)/src/github.com/nobonobo/vecty-sample; \
    make build

FROM alpine:3.7
COPY --from=build /go/src/github.com/nobonobo/vecty-sample/dist/ /dist/
WORKDIR /dist
EXPOSE 8888
